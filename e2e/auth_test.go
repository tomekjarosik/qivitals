...
func TestWorkflow_UnauthorizedNamespaceRegistration(t *testing.T) {
	...
	stdout := runCLI(t, "register --namespace finance --name budget-tracker --description 'Monthly budget' --graceful 30d --failure 35d")

	// Should fail with an error
	require.NotEmpty(t, stdout, "Register command should produce output")
	assert.Contains(t, stdout, "error", "CLI should report an error for unauthorized namespace")
}
...
func TestWorkflow_UnauthorizedNamespaceDelete(t *testing.T) {
	...
	financeID := strings.TrimSpace(runCLI(t, "register --namespace finance --name secret-budget --description 'Secret budget' --graceful 1h --failure 2h"))
	...
	out := runCLI(t, "delete --id some-uuid")
	assert.Contains(t, out, "error", "Delete should report error for unauthorized operation")
}
